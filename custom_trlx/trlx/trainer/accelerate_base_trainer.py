...
class AccelerateRLTrainer(BaseRLTrainer):
    ...
    def __init__(self, config, **kwargs):  # noqa: C901
        ...
        self.mb_count = 0        
        self.accelerator = Accelerator(log_with=config.train.tracker, project_dir=config.train.logging_dir, mixed_precision=config.train.mixed_precision)
        ...
        if self.accelerator.is_main_process:
            ...
            if config.train.tracker == "wandb":
                ...
                if config_dict["model"].get("peft_config", None):  # tensorboard does not support peft config type
                    config_dict_flat["model/peft_config"] = str(config_dict["model"]["peft_config"])
